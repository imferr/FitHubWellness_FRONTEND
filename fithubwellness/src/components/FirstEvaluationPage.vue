    ...
    <div class="first-evaluation">
        ...
        <form class="form-firstevaluation">
            ...
            <div class="save-first">
                <button class="save-button" type="button" @click="submitForm">Guardar</button>
            </div>
        </form>
    </div>
</template>
...
export default {
    setup() {
        ...
        watchEffect(() => {
            if (isAuthenticated.value && user.value && user.value.name && user.value.email) {
        ...
        const submitForm = () => {
            ...
            axios.post('http://localhost:8080/api/v1/user/createWithEvaluation', requestData)
                .then(response => {
                    Swal.fire('¡Listo!', 'Tu usuario y evaluación han sido creados', 'success');
                    router.push({ name: 'home', params: { id: response.data.userId } });
                })
                .catch(error => {
                    Swal.fire('Error', 'Hubo un error al crear el usuario y la evaluación: ' + error, 'error');
